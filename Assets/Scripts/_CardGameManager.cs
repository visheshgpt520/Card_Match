...
public class _CardGameManager : MonoBehaviour
{
    ...
    private void SetGamePanel()
    {
        ...
        float xSpacing = width / columns;
        float ySpacing = height / rows;

        float scale = 1f / gameSize;
        ...
        for (int row = 0; row < rows; row++)
        {
            for (int col = 0; col < columns; col++)
            {
                ...
                float x = -width / 2 + xSpacing * col + xSpacing / 2;
                float y = height / 2 - ySpacing * row - ySpacing / 2;
